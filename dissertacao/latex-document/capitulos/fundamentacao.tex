...
\begin{lemma}[Desigualdade de Kraft-McMillan]  \textbf{McMillan} . Para todo código binário unicamente decodificável \textbf{\emph{C} : $\emph{M} \rightarrow \emph{W}^+$} .
\begin{equation*}
...
\end{align*}
Agora vamos construir um código livre de prefixo em uma ordem crescente de tamanho, de maneira em que $l(w_i) = l_i$. Sabemos que, um código é livre de prefixo se e somente se ,existe uma palavra código $w_j$ que não contém nenhuma das palavras código anteriores $(w_1, w_2, w_{j-1})$ como prefixo.
...
\begin{align*}
&2^{l_j} \leq \sum_{i=1}^{j-1} 2^{l_j - l_i} + 1  =  2^{l_j} \leq \sum_{i=1}^{j} 2^{l_j - l_i}  \\ &=  1 \geq \sum_{i=1}^{j} 2^{l_j - l_i}  =  \sum_{i=1}^{j} 2^{l_j - l_i} \leq 1 
\end{align*}
...
\item \textbf{Desigualdade de McMillan}
...
De acordo com a desigualdade de Kraft-McMillan existe um código livre de prefixo \emph{C'} como palavras código de tamanho \emph{l(m)}, portanto:
\begin{align*}
l_a(C') &=  \sum_{m \in M', w \in W'^+}^{}p(m) l(w) \\
