...
\begin{algorithmic}
	...
	\While{$Forest.size > 1$}
		\State $T1 \gets ExtractMin(Forest)$ \Comment{Devolve a árvore cuja raiz é mínima, e retira da floresta}
...
considerando que o algoritmo de Huffman gera um código que atribui um
...
\begin{algorithmic}
	...
	\While{$cursor < M.size()$}
		\State $position, len \gets findLongestMatch(M, cursor)$ \Comment{Devolve a posição e tamanho da maior cadeia correspondente}
...
A performance das estruturas diverge entre diferentes tipos de dados testados, e apresenta \emph{tradeoffs} sobre a escolha de cada uma delas.
...
Análise de texto é uma das aplicações mais exploradas no campo dos sistemas inteligentes.
...
\end{enumerate}
 ...
 Por intuição, sabemos que o resultado com maior "agrupamento" estará no menor $k$, entretanto, 
